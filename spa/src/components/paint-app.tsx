...
import {
  ...
  FolderOpen, Grid2X2, House, ImagePlus, ListChecks, ListFilter, PackageOpen, Paintbrush, Plus, Search,
  ShoppingBasket, Sparkles, Upload, X,
} from 'lucide-react';
import { useEffect, useMemo, useRef, useState } from 'react';
import type { Paint, ShoppingItem } from '@/models/paint-model';
import type { PaintingProject } from '@/models/project-model';
...
type View = 'home' | 'marketPaints' | 'marketGames' | 'collection' | 'projects' | 'shopping' | 'imports';
...
export function PaintApp({ initialPaints, projects, shoppingSeed, config }: {
  ...
  const projectStatusLabels: Record<string, string> = {
    completed: config.projects.statuses.completed,
    in_progress: config.projects.statuses.inProgress,
    pending: config.projects.statuses.pending,
  };
  const shoppingPriorityLabels: Record<string, string> = {
    high: config.shopping.priorities.high,
    medium: config.shopping.priorities.medium,
    low: config.shopping.priorities.low,
  };
  ...
  const filteredPaints = useMemo(() => {
    ...
    return visiblePaints.filter((paint) => {
      const haystack = normalizeSearch([
        ...
        paint.reference, paint.name, paint.colorFamily, paint.colorHex, paint.finish, paint.medium, paint.opacity, paint.lifecycleStatus,
  ...
  const activeFilters = [
    ...
    ...Object.entries(filters).filter((entry): entry is [PaintFilterKey, string] => Boolean(entry[1])).map(([key, value]) => ({
      key,
      label: `${({ type: config.collection.typeFilter, color: config.collection.colorFilter, brand: config.collection.brandFilter, manufacturer: config.collection.manufacturerFilter, range: config.collection.rangeFilter, finish: config.collection.finishFilter, medium: config.collection.mediumFilter, opacity: config.collection.opacityFilter, lifecycle: config.collection.lifecycleFilter, volume: config.collection.volumeFilter, tag: config.collection.tagFilter })[key]} : ${formatMetadata(value)}`,
    })),
  ...
  async function savePaint(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const paint: Paint = {
        ...
        warnings: '',
        tags: [],
        notes: photo ? `${config.imports.photoSelected} ${photo.name}` : '',
        createdAt: now,
        updatedAt: now,
        manufacturerUrl: '',
        manufacturerImage: '',
        ...
        recommendedUses: [],
        usageInstructions: { summary: '', steps: [], tips: [] },
    ...
    <main className="min-h-screen bg-background pb-20 text-foreground lg:pb-0">
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          <button className="flex min-w-fit items-center gap-3 text-left" onClick={() => setView('home')} aria-label={`${config.brand.name} — ${config.navigation.home}`}>
      ...
      <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[224px_minmax(0,1fr)]">
        <aside className="hidden min-h-[calc(100vh-72px)] border-r px-4 py-6 lg:block">
          <nav aria-label={config.navigation.ariaLabel}>
            <NavButton icon={<House size={18} />} label={config.navigation.home} active={view === 'home'} onClick={() => setView('home')} />
        ...
        <section className="min-w-0 px-4 py-7 sm:px-6 lg:px-9 lg:py-9">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="eyebrow">{view === 'home' ? config.home.eyebrow : `${view === 'marketPaints' || view === 'marketGames' ? config.navigation.marketSection : config.navigation.workshopSection} · ${viewLabels[view]}`}</p>
              <h1 className="mt-1 text-3xl font-semibold tracking-[-0.045em] sm:text-4xl">
                {view === 'home' && config.home.title}
              ...
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                {view === 'home' && config.home.description}
          ...
          {view === 'home' && (
            <section className="mt-8">
              <div className="rounded-[30px] border bg-card p-6 shadow-sm sm:p-8">
                <div className="flex flex-wrap items-end justify-between gap-4">
                  <div><h2 className="text-xl font-semibold tracking-tight">{config.home.servicesTitle}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{config.home.servicesDescription}</p></div>
                  <div className="grid grid-cols-3 gap-2 text-center text-[10px] text-muted-foreground sm:text-xs">
                    <div className="rounded-xl bg-secondary px-3 py-2"><strong className="block text-base text-foreground">{paints.length}</strong>{config.market.paintsMetric}</div>
                    <div className="rounded-xl bg-secondary px-3 py-2"><strong className="block text-base text-foreground">{projects.length}</strong>{config.navigation.marketGames}</div>
                    <div className="rounded-xl bg-secondary px-3 py-2"><strong className="block text-base text-foreground">{projects.reduce((sum, project) => sum + project.items.reduce((count, item) => count + item.quantity, 0), 0)}</strong>{config.market.physicalItems}</div>
                  </div>
                </div>
                <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {([
                    ['marketPaints', <Grid2X2 key="market-paints" size={20} />, config.home.marketPaints],
                    ['marketGames', <BookOpen key="market-games" size={20} />, config.home.marketGames],
                    ['collection', <Droplets key="workshop-paints" size={20} />, config.home.workshopPaints],
                    ['projects', <FolderOpen key="workshop-projects" size={20} />, config.home.workshopProjects],
                    ['shopping', <ShoppingBasket key="shopping" size={20} />, config.home.shopping],
                    ['imports', <Camera key="imports" size={20} />, config.home.imports],
                  ] as [View, React.ReactNode, { title: string; description: string; action: string }][]).map(([target, icon, service]) => (
                    <button key={target} type="button" onClick={() => setView(target)} className="group rounded-[22px] border bg-background/60 p-5 text-left transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md">
                      <span className="grid size-10 place-items-center rounded-xl bg-secondary text-primary transition group-hover:bg-primary group-hover:text-primary-foreground">{icon}</span>
                      <h3 className="mt-4 text-base font-semibold">{service.title}</h3>
                      <p className="mt-2 min-h-12 text-xs leading-5 text-muted-foreground">{service.description}</p>
                      <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary">{service.action}<ChevronRight size={14} /></span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {filtersOpen && (
                <section id="paint-filters" aria-label={config.collection.filterAriaLabel} className="mt-5 rounded-[24px] border bg-card p-4 shadow-sm sm:p-5">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    <FacetSelect label={config.collection.manufacturerFilter} allLabel={config.collection.allManufacturers} value={filters.manufacturer} options={filterOptions.manufacturers} onChange={(value) => setFilter('manufacturer', value)} />
                    ...
                    <FacetSelect label={config.collection.mediumFilter} allLabel={config.collection.allMediums} value={filters.medium} options={filterOptions.mediums} onChange={(value) => setFilter('medium', value)} />
                    <FacetSelect label={config.collection.opacityFilter} allLabel={config.collection.allOpacities} value={filters.opacity} options={filterOptions.opacities} onChange={(value) => setFilter('opacity', value)} />
                    <FacetSelect label={config.collection.lifecycleFilter} allLabel={config.collection.allLifecycles} value={filters.lifecycle} options={filterOptions.lifecycles} onChange={(value) => setFilter('lifecycle', value)} />
            ...
            <div className="mt-5 grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
              ...
              <article className="min-w-0 rounded-[26px] border bg-card p-5 shadow-sm sm:p-7">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5">
                  ...
                  <span className="rounded-full bg-[#fff0da] px-3 py-1.5 text-xs font-semibold text-[#9a5217]">{projectStatusLabels[selectedProjectItem.status] ?? selectedProjectItem.status}</span>
                </div>

                <div className="mt-6 grid gap-3 lg:grid-cols-2">
                  <section className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
                    <p className="text-xs font-semibold text-primary">{config.projects.marketGuide}</p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">{config.projects.marketGuideHelp}</p>
                    {'knowledgeStatus' in selectedProjectItem.marketGuide && <span className="mt-3 inline-flex rounded-full bg-background px-2.5 py-1 text-[10px] font-semibold">{config.projects.marketGuideStatuses[selectedProjectItem.marketGuide.knowledgeStatus]}</span>}
                  </section>
                  <section className="rounded-2xl border bg-secondary/40 p-4">
                    <p className="text-xs font-semibold">{config.projects.workshopRecipe}</p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">{config.projects.workshopRecipeHelp}</p>
                    {selectedProjectItem.workshopRecipes.length === 0
                      ? <p className="mt-3 text-[11px] text-muted-foreground">{config.projects.noWorkshopRecipe}</p>
                      : <div className="mt-3 flex flex-wrap gap-2">{selectedProjectItem.workshopRecipes.map((recipe) => <span key={recipe.id} className="rounded-full bg-background px-2.5 py-1 text-[10px] font-semibold">v{recipe.version} · {recipe.status}</span>)}</div>}
                  </section>
                </div>
          ...
          <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-[28px] border bg-background shadow-2xl sm:max-w-3xl sm:rounded-[28px]">
            ...
            <div className="grid gap-6 p-5 sm:grid-cols-[280px_minmax(0,1fr)] sm:p-6">
              ...
              <div className="min-w-0">
                ...
                {(selectedPaint.usageInstructions.summary || selectedPaint.usageInstructions.steps.length > 0 || selectedPaint.usageInstructions.tips.length > 0) && (
                  <section className="mt-5 rounded-2xl border bg-background/60 p-4">
                    <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">{config.paintDetail.usageInstructions}</h3>
                    {selectedPaint.usageInstructions.summary && <p className="mt-2 text-sm leading-6">{selectedPaint.usageInstructions.summary}</p>}
                    {selectedPaint.usageInstructions.steps.length > 0 && <><p className="mt-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{config.paintDetail.usageSteps}</p><ol className="mt-2 grid gap-2">{selectedPaint.usageInstructions.steps.map((step, index) => <li key={step} className="flex gap-2 text-sm leading-5"><span className="step-number">{index + 1}</span><span>{step}</span></li>)}</ol></>}
                    {selectedPaint.usageInstructions.tips.length > 0 && <><p className="mt-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{config.paintDetail.usageTips}</p><ul className="mt-2 grid gap-1 text-sm text-muted-foreground">{selectedPaint.usageInstructions.tips.map((tip) => <li key={tip}>• {tip}</li>)}</ul></>}
                  </section>
                )}
