...
public final class WorkshopItem extends EventSourcedAggregateRoot {
    private String id;
    ...
    private WorkshopItem() {}

    public static WorkshopItem create(
            String id, String catalogItemId, String paintingProjectId,
            String displayName, int ordinal, Instant occurredAt) {
        var item = new WorkshopItem();
        item.raise(new WorkshopItemAdded(
                id, catalogItemId, paintingProjectId, displayName, ordinal, occurredAt));
        return item;
    }
    ...
    public void addComment(String comment, Instant occurredAt) {
        raise(new WorkshopItemCommentAdded(id, paintingProjectId, comment, occurredAt));
    }
    ...
    public void assignRecipe(String recipeId, int recipeVersion, Instant occurredAt) {
        raise(new WorkshopItemRecipeAssigned(id, paintingProjectId, recipeId, recipeVersion, occurredAt));
    }

    @Override
    protected void apply(DomainEvent event) {
        switch (event) {
            case WorkshopItemAdded added -> {
                id = added.workshopItemId();
            ...
            case WorkshopItemRecipeAssigned assigned -> {
            ...
            default -> throw new DomainException("invalid_workshop_item_event",
                    "Unsupported workshop item event: " + event.eventType());
        }
    }
