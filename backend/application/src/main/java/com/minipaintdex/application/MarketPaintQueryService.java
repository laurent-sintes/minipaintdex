...
/** Read-only market-paint application service over the typed Market catalog. */
final class MarketPaintQueryService {
    ...
    Stream<MarketPaintView> stream(SearchMarketPaintsQuery filters) {
        ...
        return catalogs.load().paints().stream().map(MarketPaintQueryService::view).filter(paint -> {
            ...
            if (present(filters.tag()) && paint.tags().stream()
                    .noneMatch(tag -> tag.equalsIgnoreCase(filters.tag()))) return false;
            if (!query.isBlank()) {
    ...
    private static MarketPaintView view(MarketPaint paint) {
        ...
        var technical = paint.functionalType().requiresUsageInstructions();
        ...
        return new MarketPaintView(
                ...
                paint.functionalType().id(), string(paint.reference()), paint.name(), string(paint.color().hex()),
                string(paint.finish()), string(paint.medium()), string(paint.opacity()), paint.lifecycle().id(),
                paint.dataStatus(), String.join(" · ", paint.warnings()), paint.tags(), string(paint.notes()),
                verifiedAt.isBlank() ? "" : verifiedAt + "T00:00:00.000Z",
                verifiedAt.isBlank() ? "" : verifiedAt + "T00:00:00.000Z",
                uri(paint.manufacturerPage()), string(paint.manufacturerImage().path()),
                ...
                verifiedAt, string(paint.resultImage().path()), string(paint.resultImage().credit()),
