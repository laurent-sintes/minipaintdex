...
public record PaintPotView(String paintPotId, String paintProductId, long version, PaintProductView paintProduct,
    ...
    public record Photo(String mediaId, String url, String originalUrl, String processingMethod, String caption, Instant addedAt) {}
    ...
    public static PaintPotView from(PaintPot pot, PaintProductView product) {
        return new PaintPotView(pot.id(), pot.paintProductId(), pot.version(), product,
                ...
                pot.photos().stream().map(photo -> new Photo(photo.mediaId(), photo.cutout() == null ? photo.url() : photo.cutout().url(),
                        photo.url(), photo.cutout() == null ? null : photo.cutout().processingMethod(), photo.caption(), photo.occurredAt())).toList(),
