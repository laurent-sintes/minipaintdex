...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: project ? `${project.name} — MiniPaintDex` : 'Projet introuvable' };
}
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <a href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-primary"><ArrowLeft size={16} />Retour à l’atelier</a>
        <header className="mt-7 rounded-[28px] border bg-card p-6 shadow-sm sm:p-9">
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold">
            <span className="rounded-full bg-secondary px-3 py-1.5">{project.items.length} fiches</span>
            <span className="rounded-full bg-secondary px-3 py-1.5">{miniatureCount} figurines</span>
          </div>
        ...
        <section className="mt-8">
          <h2 className="text-xl font-semibold">Fiches de peinture</h2>
