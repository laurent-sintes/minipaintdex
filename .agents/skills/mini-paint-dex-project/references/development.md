...
6. À la fin des modifications, arrêter le serveur géré avec `scripts/minipaintdex.ps1 stop`, puis exécuter le build complet avec `scripts/minipaintdex.ps1 build`. Il vérifie les tests Python et Java, le lanceur Windows, le lint, les tests et le build du frontend. Un simple lancement/redémarrage ne nécessite pas ce build complet : suivre `local-server.md`.
