...
5. Générer avec `changeset refresh-paints`. Le retrait automatique exige une référence actuellement `active`, une couverture `complete: true`, `scope: current`, une liste explicite de `ranges` et des `source_urls`. Les autres références absentes sont conservées, notamment pour une édition historique ou un statut inconnu. Une collecte historique n’est pas une preuve de disparition commerciale.
...
Les éditions commerciales sont des objets `catalog_editions` séparés dans les fichiers
de marque, référencés par les `catalog_memberships` sourcés des peintures. Une collecte
web datée ne crée pas une édition annuelle. Ajouter une édition uniquement lorsque la
publication est identifiée (titre, édition, année éventuelle, gammes et URL sources).
Conserver éditions et appartenances existantes ; ne pas déduire l’année de fabrication
