...
Paint change-set imports preserve historical source snapshots. Replacing an existing HEX or an
explicitly reviewed colour/profile field requires a new `reviewed-paint-color-quality` import
decision with matching before/after values, source URL, rationale, review ID and manifest hash.
Stale decisions fail before any write, for REST, CLI and dataset imports alike. Automatic collectors
retain protected values and union source observations; conflicts remain available for review.
These are import-evidence safeguards, not a second searchable domain model. HSL classification is
currently a deterministic administrative proposal derived from HEX, not persisted paint behaviour.
...
- Run the complete root `mvnw verify` and the deep final controls before committing. The root lifecycle must include backend tests and pnpm frontend checks. A successful full validation may be reused for identical validated content and validation context; use `scripts/minipaintdex.ps1 prepare-commit` to check the exact index, and the guarded `commit -Message` command only on an explicit commit request. Partial commits with no matching proof are validated in an isolated index snapshot, never by borrowing the dirty worktree's proof. Runtime freshness is not validation evidence. Do not rerun Python separately before Maven or restart the server when reusing a validation receipt.
